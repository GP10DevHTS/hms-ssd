<div class="max-w-lg mx-auto p-4 bg-white shadow-md rounded-lg">
    ...
    <form wire:submit.prevent="">
        <div class="mb-4">
            ...
            <input type="checkbox" wire:model.live="new_patient" id="new_patient" class="form-checkbox h-5 w-5 text-blue-600">
        </div>
        ...
        @if (!$new_patient)
            <div class="mb-4">
                ...
                @error('patient_id') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>
        @else
            <div class="mb-4">
                <label for="patient_name" class="block text-gray-700 font-medium mb-2">Name</label>
                <input type="text" wire:model="patient_name" class="form-input block w-full mt-1" id="patient_name">
                @error('patient_name') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>

            <div class="mb-4">
                <label for="patient_email" class="block text-gray-700 font-medium mb-2">Email</label>
                <input type="email" wire:model="patient_email" class="form-input block w-full mt-1" id="patient_email">
                @error('patient_email') <span class="text-red-600">{{ $message }}</span> @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" wire:model="date_of_birth" class="form-input block w-full mt-1" id="date_of_birth">
        ...
        <div class="mb-4">
            ...
            <input type="date" wire:model="appointment_date" class="form-input block w-full mt-1" id="appointment_date">
            @error('appointment_date') <span class="text-red-600">{{ $message }}</span> @enderror
        </div>
        ...
        <button wire:click="createAppointment" class="bg-blue-600 text-white font-bold py-2 px-4 rounded hover:bg-blue-700">Create Appointment</button>
    </form>
</div>
